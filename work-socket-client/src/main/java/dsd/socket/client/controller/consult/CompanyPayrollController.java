...
import dsd.socket.client.view.consult.CompanyPayrollView;
import java.util.ArrayList;
import java.util.List;
import java.util.Map;

public final class CompanyPayrollController implements BaseCosultController {

    private final CompanyPayrollView view;
    private final CompanyService companyService;

    public CompanyPayrollController() {
        this.view = new CompanyPayrollView();
    ...
    public void fillTable() {
        ...
        for (Map.Entry<Integer, Company> entry : allCompanies.entrySet()) {
            values.add(new Object[]{entry.getValue().getId(),
                ...
                ((CompanyService) companyService).payroll(entry.getValue().getId())
            });
        }
        try {
