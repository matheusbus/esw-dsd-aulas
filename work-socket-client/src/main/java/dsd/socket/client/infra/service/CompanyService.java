...
public class CompanyService implements Service<Company, Integer> {
    ...
    public Company find(Integer id) {
        ...
        List<Person> peopleCompany = null;
        if (responseData[4].equals("[]")) {
            peopleCompany = new ArrayList<>();
        } else {

            String[] peopleList = responseData[4].split(",");

            for (String peopleList1 : peopleList) {
                String[] personData = peopleList1.split(";");
            }

        }

        Company company = new Company(idCompany, cnpjCompany, socialReasonCompany, foundedIn, peopleCompany);
        return company;
    }
